...
def entry_scan_text(row,signal=None,reason=None):
    """Explain the aligned direction of each strategy on ONE closed-candle line."""
    def finite(x):return isinstance(x,(int,float)) and math.isfinite(x)
    def describe(report,label,default_periods):
        fast,slow=report.get('ema_fast'),report.get('ema_slow')
        fp,sp=report.get('ema_fast_period',default_periods[0]),report.get('ema_slow_period',default_periods[1])
        if not finite(fast) or not finite(slow):return f'{label} EMA {fp}/{sp}: warm-up incomplete',None
        side=1 if fast>slow else -1 if fast<slow else 0
        trend={1:'BULLISH',-1:'BEARISH',0:'FLAT'}[side]
        return f'{label} EMA {fp}/{sp} {trend} ({fast:.2f}/{slow:.2f})',side
    details=row.get('diagnostics')
    server=isinstance(details,dict) and details.get('signal_ms')==row['time']
    report=details if server else row
    ema_text,side=describe(report,'CORE' if server else 'LOCAL',(34,89))
    when=datetime.fromtimestamp(row['time']/1000,timezone.utc).strftime('%Y-%m-%d %H:%M UTC')
    parts=[f'BTC/USDC 5m closed {when}',ema_text]
    if reason or row.get('entry_block'):
        parts.append('ENTRY BLOCKED: '+str(reason or row['entry_block']))
        return ' | '.join(parts)
    def explain(d,direction,selected):
        if not direction:return 'BUY/SELL BLOCKED: EMA alignment is flat or incomplete'
        label='BUY' if direction==1 else 'SELL';blocks=d.get('buy_blocks' if direction==1 else 'sell_blocks')
        if not isinstance(blocks,list) or any(not isinstance(x,str) for x in blocks):return label+' BLOCKED: server filter details unavailable'
        if blocks:return label+' BLOCKED: '+'; '.join(blocks)
        if selected==direction:return label+' FILTERS PASSED; execution checks pending'
        return label+' BLOCKED: signal/diagnostic mismatch; no reason will be guessed'
    if server:parts.append(explain(details,side,signal))
    elif signal:parts.append(('BUY' if signal==1 else 'SELL')+' FILTERS PASSED; execution checks pending')
    else:parts.append('ENTRY BLOCKED: private filter reasons unavailable; update the server')
    extra=row.get('supplementary',{});diag=extra.get('diagnostics')
    if isinstance(diag,dict) and diag.get('signal_ms')==row['time']:
        label,direction=describe(diag,'SUPPLEMENTARY',(13,34));parts.append(label)
        if row.get('supplementary_block'):parts.append('ENTRY BLOCKED: '+row['supplementary_block'])
        else:parts.append(explain(diag,direction,extra.get('signal')))
    elif server:parts.append('SUPPLEMENTARY: filter reasons unavailable; update the server')
    return ' | '.join(parts)
...
class Agent:
            ...
            if result.status=='filled' and result.qty>0 and result.price>0:
                ...
                if p is None:
                    ...
                    p={'side':c['side'],'qty':result.qty,'first':price,'opened':c['created'],
                       ...
                       'origin':c.get('origin','core'),'stop_config':c.get('stop_config',self.stop_cfg.to_dict())}
                    if p['origin']=='supplementary':p['additions_disabled']=True
    ...
    def reconcile(self):
        ...
        if abs(a.qty)<=tol:
            ...
            self.state['last_closed_origin']=p.get('origin','core')
            if p.get('origin')=='supplementary':self.state['last_supplementary_exit_bar']=closed_time//BAR_MS*BAR_MS
    ...
    def open_entry(self,row,is_add=False,level=0,fixed_qty=None,fixed_equity=None):
        ...
        if not is_add:
            if a.qty or self.broker.orders():
                self.log.warning('NO OPEN ENTRY | %s',entry_scan_text(row,reason='BTC already has a position or open orders; account must be reconciled'));return False
        ...
        try:
            ...
            self.log.warning('NO OPEN ENTRY | %s',entry_scan_text(row,reason=str(e)));return False
        hook=getattr(self,'before_entry',None)
        if hook and not hook(row,side,qty,distance,is_add,level):
            self.log.warning('NO OPEN ENTRY | %s',entry_scan_text(row,reason=row.get('entry_block','pre-entry authorization failed; see preceding error')))
            return False
        c=self.make_command('entry',side=side,qty=qty,reference=self.quote['mid'],equity=equity,
            ...
            origin=row.get('entry_origin','core'),stop_config=(SUPPLEMENTARY_STOP if row.get('entry_origin')=='supplementary' else self.stop_cfg.to_dict()))
        outcome=self.send(c)
        if outcome.status not in ('filled','unknown'):
            self.log.warning('NO OPEN ENTRY | %s',entry_scan_text(row,reason='exchange order '+outcome.status+': '+str(outcome.message)))
    ...
    def poll(self):
        ...
        if p:
            ...
            self.event('POSITION %s | %.5f BTC | price %.2f | move %+.2f%% | confirmed stop %s | trailing %s | indicators %s',
                ...
                'active' if p['trail']['active'] else f'waiting for +{self.position_stop(p).activate_profit_pct:g}%',NAMES[p['trail']['mode']])

    def position_stop(self,position):
        cfg=StopConfig(**position.get('stop_config',self.stop_cfg.to_dict()))
        cfg.validate();return cfg

    def resume_core_entry(self):
        intent=self.state.get('core_handoff')
        if not intent:return
        age=self.now()-intent['time']
        if not 0<=age<=self.settings['max_edad_senal_segundos']*1000:
            self.state['core_handoff']=None;self.save()
            self.event('CORE HANDOFF EXPIRED | no historical entry will be replayed');return
        if self.state['position'] or self.state['pending'] or self.state['cleanup'] or self.state['blocked']:return
        self.state['core_handoff']=None;self.save()
        self.event('CORE PRIORITY | supplementary close confirmed; submitting the fresh core entry')
        self.open_entry(intent)
    ...
    def process_rows(self,rows):
        ...
        for row in new:
            ...
            if p and row['time']>p['opened']:
                trail=Trailing(**p['trail']).advance(row['close'],row['atr'],row['raw_long'] if p['side']==1 else row['raw_short'],self.position_stop(p))
        ...
        p=self.state['position']
        if p:
            if p.get('origin')=='supplementary':
                if p['emergency']:return
                decision=self.entry_decision(last)
                if decision is not None:
                    signal,risk_atr=decision
                    if signal and math.isfinite(risk_atr):
                        intent=dict(last,signal=signal,risk_atr=risk_atr,entry_origin='core')
                        self.state['core_handoff']=intent;self.save()
                        self.event('CORE PRIORITY | closing supplementary position before core %s', 'BUY' if signal==1 else 'SELL')
                        self.emergency_close('Core entry takes priority over supplementary position')
                        self.reconcile();self.cleanup();self.resume_core_entry()
                return
            ...
            # Match closed-candle replay: a bar that closed a position cannot re-enter.
            if last['time']-BAR_MS<=self.state['closed_at']:
                self.event('NO OPEN POSITIONS | %s',entry_scan_text(last,reason='a position closed during this candle; same-candle re-entry is disabled'))
                return
            decision=self.entry_decision(last)
            if decision is None:
            ...
            signal,risk_atr=decision
            origin='core'
            extra=last.get('supplementary',{})
            if not signal and extra.get('signal') in (-1,1):
                waited=last['time']-BAR_MS-self.state.get('last_supplementary_exit_bar',-10**15)
                distance=extra.get('distance',float('nan'))
                if waited>=SUPPLEMENTARY_COOLDOWN_BARS*BAR_MS and math.isfinite(distance) and distance>0:
                    signal=int(extra['signal']);risk_atr=distance/self.mgmt.stop_atr;origin='supplementary'
                else:
                    if waited<SUPPLEMENTARY_COOLDOWN_BARS*BAR_MS:
                        remaining=max(0,SUPPLEMENTARY_COOLDOWN_BARS-math.floor(waited/BAR_MS))
                        last['supplementary_block']=f'cooldown: {remaining} closed 5m candles remaining'
                    else:last['supplementary_block']='initial stop distance unavailable or invalid'
            last['entry_origin']=origin
            if origin=='supplementary':
                self.event('SUPPLEMENTARY ENTRY | confirmed breakout %s | core strategy is currently flat', 'BUY' if signal==1 else 'SELL')
            reason='initial-stop ATR unavailable' if signal and not math.isfinite(risk_atr) else None
            if origin=='supplementary':
                self.event('ENTRY READY | BTC/USDC 5m | supplementary %s | confirmed range breakout; original strategy is flat', 'BUY' if signal==1 else 'SELL')
            else:
...
VERSION='2.2.0'
BUILD_ID='2.2.0-supplementary.2'
STRATEGY_PROTOCOL='qts-core-plus-supplementary-v1'
SUPPLEMENTARY_PROFILE='supplementary-breakout-v1'
SUPPLEMENTARY_STOP={'atr_period': 14, 'wide_atr': 24.0, 'wide_min_pct': 0.4, 'weak_ratio': 0.7, 'adverse_ratio': 0.4, 'activate_profit_pct': 2.0, 'confirm_bars': 3, 'rsi_favorable': 55.0, 'rsi_drop_points': 3.0, 'momentum_bars': 3, 'atr_expansion_min': 0.9, 'tighten_before_activation': False}
SUPPLEMENTARY_COOLDOWN_BARS=24
...
def build_payment_header(cfg,key,requirements_json):
    ...
    # The challenge tag carries no permission and changes nothing that is signed.
    allowed_extra=({'feePayer':cfg.fee_payer},{'feePayer':cfg.fee_payer,'tag':'x402-global-challenge'})
    if (req.scheme!='exact' or req.network!=cfg.chain or req.asset!=cfg.asset or
        req.pay_to!=cfg.pay_to or req.amount!=str(cfg.price) or req.extra not in allowed_extra or
...
class PaidAudit:
        ...
        try:
            if not header:
                ...
                if response.status_code==200:
                    # Already settled for this event and local state was lost.
                    data=response.json();settle=qts_unb64(response.headers.get('PAYMENT-RESPONSE',''))
                    if (data.get('id')!=event_id or data.get('payer')!=self.cfg.payer or
                        settle.get('success') is not True or settle.get('network')!=self.cfg.chain or
                        settle.get('payer')!=self.cfg.payer or
                        settle.get('transaction')!=data.get('transaction')):
                        raise SafetyError('Invalid payment receipt for an already-settled event')
                    with self.db:self.db.execute('UPDATE pagos SET stage=?,tx=?,receipt=? WHERE id=?',
                        ('paid',data.get('transaction'),canonical(data),event_id))
                    self.log.info('X402 ALREADY PAID | event %s | transaction %s',event_id,data.get('transaction'))
                    return data
                if response.status_code!=402:
                    raise TemporaryError('Service did not return the expected x402 fee (HTTP '
    ...
    def entry_signal(self,row):
        ...
        try:
            # A cold recomputation on a small instance takes longer than 12s. The
            # agent has a whole 5-minute candle before the next decision, so it can
            # afford to wait; timing out early just skipped every fresh candle.
            response=self.session.post(self.cfg.endpoint+'/v1/signal/BTC',data=raw,headers=headers,timeout=(5,50),allow_redirects=False)
        ...
        if data.get('strategy_protocol')!=STRATEGY_PROTOCOL:
            raise Rejected('Server strategy version is not compatible; update the private server first')
        extra=data.get('supplementary',{})
        if extra.get('profile')!=SUPPLEMENTARY_PROFILE or extra.get('signal') not in (-1,0,1):
            raise Rejected('Malformed supplementary signal or unknown stop profile')
        distance=extra.get('distance')
        if extra.get('signal') and (not isinstance(distance,(int,float)) or not math.isfinite(distance) or distance<=0):
            raise Rejected('Supplementary initial stop is not valid')
        row['supplementary']=extra
...
def configure_wizard(cfg,input_fn=input,password_fn=getpass.getpass,vault=None,api=None):
    ...
    print('Run INICIAR.bat. config.txt contains only public data; keys remain protected.')
    return cfg
...
class AuditService:
    ...
    def requirement(self):
        ...
        return PaymentRequirements(scheme='exact',network=self.cfg.chain,asset=self.cfg.asset,
            ...
            # 'tag' attributes settled volume to the Global x402 Challenge leaderboard.
            extra={'feePayer':self.cfg.fee_payer,'tag':'x402-global-challenge'})

    BAZAAR_DISCOVERY={'bazaar':{'info':{'input':{'type':'http','bodyType':'json',
        'body':{'kind':'activation','payer':'ALGORAND_ADDRESS','id':'EVENT_ID'}},
        'output':{'type':'json','example':{'id':'EVENT_ID','payer':'ALGORAND_ADDRESS',
            'transaction':'ALGORAND_TXID','result':{'allowed':True}}}},
        'schema':{'$schema':'https://json-schema.org/draft/2020-12/schema','type':'object',
            'properties':{'input':{'type':'object','properties':{
                'type':{'type':'string','const':'http'},
                'method':{'type':'string','enum':['POST','PUT','PATCH']},
                'bodyType':{'type':'string','enum':['json','form-data','text']},
                'body':{'properties':{'kind':{'type':'string'},'payer':{'type':'string'},
                                      'id':{'type':'string'}},'required':['kind','payer','id']}},
                'required':['type','bodyType','body'],'additionalProperties':False},
                'output':{'type':'object','properties':{'type':{'type':'string'},
                          'example':{'type':'object'}},'required':['type']}},
            'required':['input']}}}

    def quote(self):
        from x402.schemas import PaymentRequired,ResourceInfo
        return PaymentRequired(accepts=[self.requirement()],extensions=self.BAZAAR_DISCOVERY,
            resource=ResourceInfo(url=self.cfg.endpoint+'/v1/audit',
            description='Algorithmic BTC/USDC trading signals for Hyperliquid: one-time agent '
                        'activation, then a settlement receipt per closed position, returning the '
                        'entry direction and stop-risk ATR the agent cannot compute locally',
    ...
    def evaluate_signal(self,payer,feed,signal_ms):
        ...
        if hasattr(feed,'packet'):
            packet=feed.packet(signal_ms)
            if packet is None:raise ServiceError(503,'Supplementary strategy is not ready')
            distance=packet['supplementary_distance']
            result['strategy_protocol']=STRATEGY_PROTOCOL
            result['supplementary']={'signal':packet['supplementary_signal'],
                'distance':distance if math.isfinite(distance) else None,
                'profile':packet['supplementary_profile'],'diagnostics':packet.get('diagnostics')}
...
class SignalFeed:
    ...
    def __init__(self,cfg,api=None,now=None,min_gap_ms=15_000,strategy=None):
        ...
        self.lock=threading.Lock();self.table={};self.reports={};self.supplementary_table={};self.fetched=0
        # Held only while the heavy recomputation runs, so readers never block on it.
        self.refresh_lock=threading.Lock()
        self.warm_thread=None

    def start_warming(self):
        """Refresh right AFTER each candle close, not on a blind timer.

        A fixed 15s cycle kept landing just BEFORE a close, so the table was
        missing exactly the candle the agent was about to ask for.
        """
        import threading
        log=logging.getLogger('quant_trading_signals.senal')
        def work():
            while True:
                try:
                    self.refresh()
                except Exception as error:
                    log.warning('BINANCE SIGNAL WARMUP | %s | %s',type(error).__name__,error)
                now=self.now()
                # wake 3s after the next candle closes, so the fresh candle is in
                # the table before the agent polls for it
                wait=((now//BAR_MS+1)*BAR_MS+3000-now)/1000
                time.sleep(max(2.,min(wait,BAR_MS/1000)))
    ...
    def refresh(self):
        """Rebuild the table. Runs WITHOUT self.lock so readers are never blocked;
        ...
        try:
            ...
            if hasattr(self.strategy,'entry_report'):
                table,reports=self.strategy.entry_report(data,manifest,report_times=recent)
            else:
                table,reports=self.strategy.entry_rows(data,manifest),{}
            import complementary
            supplementary=complementary.entry_packets(data,manifest,report_times=recent)
            with self.lock:
                self.table,self.reports,self.supplementary_table=table,reports,supplementary
    ...
    def decision(self,signal_ms):
        ...
        Answers from the cached table. If the requested candle is not there yet,
        refresh once and answer from the rebuilt table; if another refresh is
        already running, wait for it rather than starting a second one.
        """
        with self.lock:
            answer=self.table.get(int(signal_ms))
        if answer is not None:
            return answer
        if not self.refresh():
            # a refresh was already in flight: wait for it, then re-read
            with self.refresh_lock:pass
        with self.lock:
            return self.table.get(int(signal_ms))

    def packet(self,signal_ms):
        self.decision(signal_ms)
        with self.lock:return self.supplementary_table.get(int(signal_ms))
...
def main(argv=None):
    ...
    try:
        ...
        print('\nSetup cancelled; incomplete setup is not marked complete.' if args.configurar else '\nAgent stopped. Restart from the same folder with INICIAR.bat.');return 130
        ...
        print('CANNOT CONTINUE |',type(error).__name__,'| credentials are not displayed. Check connectivity and the error messages above.');return 1
