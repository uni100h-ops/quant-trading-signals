...
4. Deploy the SAME embedded source using `python agente.py --servir`, with `estado_servidor/` persistent and an HTTPS reverse proxy. `Dockerfile` is supplied. On Render the service reads the assigned `PORT` and binds `0.0.0.0` automatically; `config.txt`'s host/port are used only for local runs. Run one instance unless you replace SQLite with appropriate shared transaction locking.
